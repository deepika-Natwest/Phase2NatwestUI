...
const CATEGORY_OPTIONS = ["Cost Saving", "Process Improvement", "New Functionality"];
...
function DeliverablePage() {
  ...
  const [bulkEditData, setBulkEditData] = useState({ capabilityId: "", franchiseId: "", category: "" });
  ...
  const openModal = (item = null) => {
    setCurrentItem(item);
    setFormData(item || {
      deliveryTitle: "", capabilityId: "", franchiseId: "", category: "",
      aiBased: false, projectName: "", description: "", resources: [],
      costSavingAmount: "", costSavingCurrency: "GPP",
      improvementType: "", timeHours: "", timeMinutes: "", percentage: "",
      newFunctionality: "",
    });
    setModalOpen(true);
  };
  ...
  const handleChange = (e) => {
    ...
    if (name === "category") {
      setFormData({ ...formData, category: value, improvementType: "", costSavingAmount: "", timeHours: "", timeMinutes: "", percentage: "", newFunctionality: "" });
      return;
    }
    if (name === "improvementType") {
      setFormData({ ...formData, improvementType: value, timeHours: "", timeMinutes: "", percentage: "" });
      return;
    }
  ...
  const filteredFranchises = franchises.filter((f) => f.capabilityId === formData.capabilityId);

  // ── Bulk delete helpers ───────────────────────────
  const allSelected =
    deliverables.length > 0 && deliverables.every(d => selectedIds.has(d.id));
  const someSelected =
    deliverables.some(d => selectedIds.has(d.id)) && !allSelected;
  ...
  return (
    <Layout>
      ...
      {/* Bulk delete toolbar — shown only when rows are selected */}
          ...
          <div className="d-flex justify-content-center">
            {!confirmDelete ? (
              <div className="d-inline-flex gap-2">
                ...
                {hasAnyRole(role, [ROLES.ADMIN, ROLES.EDITOR]) && (
                  <button className="btn btn-info" onClick={() => { setBulkEditData({ capabilityId: "", franchiseId: "", category: "" }); setBulkEditOpen(true); }}>
              ...
              <div className="d-inline-flex align-items-center gap-2
                ...
                <span className="text-danger fw-semibold" style={{ fontSize: "14px" }}>
                  Delete {selectedCount} deliverable{selectedCount !== 1 ? "s" : ""}?
                </span>
      ...
      <div className="adminContent p-4">
        <table className="table table-bordered table-striped">
          ...
          <tbody>
            {deliverables.map((d) => (
              <tr key={d.id} style={selectedIds.has(d.id) ? { background: "#fff1f1" } : undefined}>
                <td>
                  <input
                    type="checkbox"
                ...
                <td>
                  {hasAnyRole(role, [ROLES.ADMIN, ROLES.EDITOR]) && (
                    <button className="btn btn-sm btn-warning me-2" onClick={() => openModal(d)}>
                      Edit
                    </button>
                  )}
            ...
            {deliverables.length === 0 && (
              <tr>
                <td colSpan="6" className="text-center text-muted py-4">No deliverables found.</td>
              </tr>
            )}
          </tbody>
        </table>
            ...
            <div className="modal-content">
              <form onSubmit={handleSubmit}>
                <div className="modal-header">
                  <h5 className="modal-title">{currentItem ? "Edit Deliverable" : "Add Deliverable"}</h5>
                  <button className="btn-close" onClick={closeModal} />
                </div>
                <div className="modal-body row">
                  <div className="col-md-12 mb-3">
                    <label>Title</label>
                    <input type="text" name="deliveryTitle" className="form-control" value={formData.deliveryTitle} onChange={handleChange} required />
                  </div>
                  <div className="col-md-6 mb-3">
                    <label>Capability</label>
                    <select name="capabilityId" className="form-control" value={formData.capabilityId} onChange={handleChange} required>
                  ...
                  <div className="col-md-6 mb-3">
                    ...
                    <select name="franchiseId" className="form-control" value={formData.franchiseId} onChange={handleChange} required>
                      <option value="">Select</option>
                      {filteredFranchises.map((f) => <option key={f.id} value={f.id}>{f.name}</option>)}
                    </select>
                  </div>
                  <div className="col-md-6 mb-3">
                  ...
                  <div className="col-md-6 mb-3 d-flex align-items-end">
                    <div className="form-check">
                      <input type="checkbox" className="form-check-input" name="aiBased" checked={formData.aiBased} onChange={handleChange} />
                      <label className="form-check-label">AI Based Project</label>
                    </div>
                  </div>
                  ...
                  {formData.category === "Cost Saving" && (
                    <div className="col-md-6 mb-3">
                      ...
                      <input type="number" name="costSavingAmount" className="form-control" value={formData.costSavingAmount} onChange={handleChange} required />
                    </div>
                  )}
                  ...
                  {formData.category === "Process Improvement" && (
                    <>
                      <div className="col-md-6 mb-3">
                        ...
                        <select name="improvementType" className="form-control" value={formData.improvementType} onChange={handleChange} required>
                          <option value="">Select</option>
                      ...
                      {formData.improvementType === "Time" && (
                        <>
                          <div className="col-md-6 mb-3">
                            ...
                            <input type="number" name="timeHours" className="form-control" value={formData.timeHours} onChange={handleChange} required />
                          </div>
                          <div className="col-md-6 mb-3">
                            <label>Time Saved (Minutes)</label>
                            <input type="number" name="timeMinutes" className="form-control" value={formData.timeMinutes} onChange={handleChange} min="0" max="59" required />
                          </div>
                        </>
                      )}
                      {formData.improvementType === "Percentage" && (
                        <div className="col-md-6 mb-3">
                          ...
                          <input type="number" name="percentage" className="form-control" value={formData.percentage} onChange={handleChange} min="0" max="100" required />
                        </div>
                      )}
                    </>
                  )}
                  ...
                  {formData.category === "New Functionality" && (
                    <div className="col-12 mb-3">
                      ...
                      <textarea name="newFunctionality" className="form-control" rows="3" value={formData.newFunctionality} onChange={handleChange} required />
                    </div>
                  )}

                  <div className="col-md-6 mb-3">
                    <label>Project Name</label>
                    <input type="text" name="projectName" className="form-control" value={formData.projectName} onChange={handleChange} required />
                  </div>
                  <div className="col-md-6 mb-3">
                    <label>Resources</label>
                    <select multiple className="form-control" value={formData.resources} onChange={handleResourceChange}>
      ...
      {bulkEditOpen && (
        <div className="modal show fade d-block" style={{ backgroundColor: "rgba(0,0,0,0.5)" }}>
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Bulk Edit — {selectedCount} deliverable{selectedCount !== 1 ? "s" : ""}</h5>
              ...
              <div className="modal-body">
                ...
                <div className="mb-3">
                  ...
                  <select className="form-select" value={bulkEditData.category}
              ...
              <div className="modal-footer">
                ...
                <button className="btn btn-primary" onClick={handleBulkEdit} disabled={bulkEditing}>
                  {bulkEditing ? "Saving…" : `Apply to ${selectedCount} deliverable${selectedCount !== 1 ? "s" : ""}`}
                </button>
              </div>
            </div>
          </div>
